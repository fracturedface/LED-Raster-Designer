class CanvasRenderer {
    ...
    handleKeyDown(e) {
        ...
        // Cmd/Ctrl+0 - Fit to view (standard image editors standard)
        if ((e.metaKey || e.ctrlKey) && e.code === 'Digit0' && !isTyping) {
            e.preventDefault();
            this.fitToView();
        }

        // Cmd/Ctrl+1 - Actual size 1:1 (standard image editors standard)
        if ((e.metaKey || e.ctrlKey) && e.code === 'Digit1' && !isTyping) {
            e.preventDefault();
            this.zoomActual();
        }

        // Cmd/Ctrl+; - Toggle snap (standard image editors standard)
